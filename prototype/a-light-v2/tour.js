...
(() => {
  ...
  /* ---------- C-01 · Mobile menu ----------
     Identical behaviour to the homepage; the panel's top comes from the nav's
     own rect so it lands under the bar in every scroll position. */
  const menuBtn = document.querySelector('[data-menu-btn]');
  const menu = document.querySelector('[data-menu]');
  const navEl = document.querySelector('[data-nav]');

  if (menuBtn && menu && navEl) {
    const place = () => { menu.style.top = Math.max(0, navEl.getBoundingClientRect().bottom) + 'px'; };
    const setOpen = (open) => {
  ...
  /* ---------- A-02 · Progressive disclosure ----------
     The previous rail capped its own height and scrolled its middle, which hid
     630px of the form at 1440×900 with no affordance that anything was there.
     One step is open at a time instead: choosing a date collapses the calendar
     to an editable line and reveals travellers and extras in the space it
     leaves, so the card fits the viewport without a second scrollbar. */
  const stepDate = document.querySelector('[data-step="date"]');
  const picked = document.querySelector('[data-edit-date]');
  const laterSteps = [...document.querySelectorAll('[data-step="party"], [data-step="extras"]')];

  function showSteps(open) {
    if (!stepDate || !picked) return;
    stepDate.hidden = open;
    picked.hidden = !open;
    laterSteps.forEach(el => { el.hidden = !open; });
  }

  picked?.addEventListener('click', () => {
    showSteps(false);
    // Put the reader back on the control they asked to change.
    grid.querySelector('button[aria-pressed="true"]')?.focus();
  });
  ...
  function sync() {
    ...
    // The collapsed date line reads from the same state as everything else.
    if (state.date) {
      const d = dateObj();
      const pd = $('[data-picked-date]');
      if (pd) pd.textContent = SHORT.format(d);
    }
  ...
  document.querySelector('[data-pay]').addEventListener('click', () => {
    ...
    // M-01 · Focus and a tinted border read as focus, not as an error, and
    // announced nothing at all. Each field now states the problem in place.
    const fail = (field, id) => {
    ...
    const clear = (field, id) => {
      field.removeAttribute('aria-invalid');
      const msg = document.getElementById(id);
      if (msg) msg.hidden = true;
    };

    clear(name, 'co-name-err');
    clear(email, 'co-email-err');
    if (!name.value.trim()) { fail(name, 'co-name-err'); return; }
    if (!email.checkValidity()) { fail(email, 'co-email-err'); return; }
  ...
  /* C-02 · This observed the rail and gated on scrollY. On mobile the rail
     starts 5,016px down, so it began un-intersecting: the callback ran once,
     read scrollY as 0, resolved false, and — because the rail's intersection
     state never changed on the way down — never ran again. The bar was hidden
     at every scroll position on the page.

     It now watches a sentinel just under the gallery, which the reader really
     does cross, and hides again once the rail itself is on screen so two live
     totals never compete for the same tap. */
  const bar = document.querySelector('[data-bookbar]');
  const barSentinel = document.querySelector('[data-bar-sentinel]');
  const railEl = document.querySelector('.rail');

  if (bar && barSentinel && railEl) {
    let past = false, atRail = false;
    const paint = () => bar.setAttribute('data-show', String(past && !atRail));

    new IntersectionObserver(([e]) => {
      past = e.boundingClientRect.top < 0;
      paint();
    }, { threshold: 0 }).observe(barSentinel);

    new IntersectionObserver(([e]) => {
      atRail = e.isIntersecting;
      paint();
