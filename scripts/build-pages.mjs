...
async function buildDirection(dir) {
  ...
  for (const file of htmlFiles) {
    const authored = sanitise(await readFile(path.join(from, file), 'utf8'));

    // Scan what a human wrote, then rewrite the asset names — not the other way
    // round. The hashes injected below are generated by this script, and a hash
    // that happens to contain eight digits starting with 2, 4, 6, 7 or 8 reads as
    // a Costa Rican phone number to the guard: `styles.c967224956.css` refused a
    // build once. Nothing is lost by scanning first, because the substitution only
    // ever rewrites a filename this build chose itself.
    assertPublishable(authored, `${dir}/${file}`);
    assertHtmlSafe(authored, `${dir}/${file}`);
